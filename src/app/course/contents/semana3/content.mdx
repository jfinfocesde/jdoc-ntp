...
En Python, las estructuras de control repetitivas se utilizan para ejecutar una o más instrucciones de manera repetitiva mientras se cumpla una condición determinada. Las tres estructuras de control repetitivas en Python son: Bucle while, Bucle for
...
Las funciones lambda en Python son funciones anónimas, es decir, no tienen un nombre definido y se crean en una sola línea de código. Estas funciones se definen con la palabra clave "lambda" seguida de los parámetros entre paréntesis, luego ":" y la expresión que se debe evaluar.
...
```
Aquí hay un ejemplo simple de una función lambda que devuelve el doble de un número:

```python  
doble = lambda x : x * 2
```

En este ejemplo, se crea una función lambda llamada "doble" que toma un parámetro "x" y devuelve el resultado de multiplicar "x" por 2.

Las funciones lambda se pueden utilizar en lugar de funciones regulares en situaciones donde solo se necesita una función simple y no se necesita definir una función con un nombre específico. Por ejemplo, se pueden utilizar en combinación con funciones como map(), filter() o reduce() para aplicar una operación a cada elemento de una lista o filtrar elementos que cumplan una condición.

## Ejemplo

1. Crear una función lambda que calcule el cuadrado de un número:

```python  
cuadrado = lambda x: x**2
```

2. Crear una función lambda que tome dos argumentos y devuelva el mayor de ellos:

```python  
mayor = lambda x, y: x if x > y else y
```

3. Crear una función lambda que tome una lista de números y devuelva una lista con los números elevados al cuadrado:

```python  
lista_cuadrados = lambda lista: list(map(lambda x: x**2, lista))
```

4. Crear una función lambda que tome una lista de cadenas y devuelva una lista con la longitud de cada cadena:

```python  
longitudes = lambda lista: list(map(lambda x: len(x), lista))
```

5. Convertir una lista de temperaturas de Celsius a Fahrenheit:
